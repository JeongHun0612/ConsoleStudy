...
- C/C++ 언어는 {} 의 조합을 통해서 특정 영역의 시작과 끝을 나타내는것이 가능하다.
...
- 면접 사골 문제
 ㄴ 지역, 전역, 동적 변수 등 변수의 LifeCycle 설명해봐라.
  ㄴ 메모리 순환 질문
...
void main()
{
	...
	// 0
	cout << "전역 변수의 값 : " << g_Value << endl << endl;
	// - static 키워드의 영향으로 함수 호출시 값이 누적이 된다.

	/*
	C/C++ 언어는 특정 영역안에 새로운 영역을 만드는 기능을 제공한다.
	 ㄴ 영역 중첩이 가능하다.

	또한, C/C++ 언어는 영역만 중복되지 않는다면 동일한 이름을 지니는 변/상수를 만드는것이 가능하기 때문에
	 특정 영역과 해당 영역의 새로운 지역 영역에서 동일한 이름의 변수를 만들었다고 하더라도 이는 컴파일러 입장에서 에러로 처리하지 않는다.
	  ㄴ 문법적으로 해당 기능을 제공

	단, 지역 영역에서 새로운 변수를 만들었다 하더라도 해당 변수의 이름과 동일한 이름을 지니는 변수가
	외부 영역에서 선언되어 있으면 특정 지역을 지정해서 해당 변수의 값을 변경하는 것이 불가능

	Why -> 변/상수의 접근 우선 순위는 가장 가까운 영역으로 범위가 제한되기 때문에
	*/

	// 익명 변수, 익명 메소드 등등 - 일회성 데이터
	{ // <- 새로운 영역 시작
		int Value = 0;
...
void DecreaseValue(int Value)
{
	...
	/*
	정적 변/상수는 특정에서만 접근이 가능한 지역 변수의 특정과 프로그램이 종료될때까지 사라지지 않는 전역 변수의 특성을 모두 지니고 있다.

	EX) 지역 변수 <=> 전역 변수

	그렇기 때문에 정적 변/상수를 선언하는 해당 코드는 해당 영역이 실행되는 그 첫 순간에만 유효하다는것을 알 수 있다. (여기선 초기화가 한번만 이뤄진다.)
	*/
}
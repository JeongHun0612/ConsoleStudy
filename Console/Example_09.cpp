...
- 순서 : 전처리기 -> 컴파일러 -> 어셈블러 -> 링커
 ...
 전처리기 (Preprocessor)
  ㄴ 헤더 파일 삽입, 코드의 주석 제거, 문법 검토, 매크로 등을 치환(바꾸다)하는 역할을 수행한다.
...
▶ 선행처리를 위한 명령
 ...
 ㄴ #include "파일 이름" -> 현재의 소스 코드가 저장되어 있는 폴더에서 먼저 찾고 파일이 없다면 미리 정의된 include 폴더에서 파일을 찾는다.
...
★ 면접 사골 문제
 1. #define
  ...
  ㄴ 매크로 VS 상수 
    ㄴ 상수가 압도적으로 좋다. 이유는 ? 키워드 (컴파일 시간 , 메모리 사용량)
...
/*
 ...
 6. 매크로 상수는 단순히 컴파일전에 코드를 튜닝하기 위한 전처리기에 의해서 처리되는 상수이기 때문에
    컴파일러 입장에서는 리터럴 상수와 동일시 된다.
     ㄴ 리터럴 상수와 동일시 -> 컴파일러는 매크로 상수의 존재를 알 수 없다.
*/
#pragma endregion

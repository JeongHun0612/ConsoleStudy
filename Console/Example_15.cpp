...
- 메모리의 주소를 의미하며 C / C++ 언어는 포인터 문법을 통해서 데이터에 간접적으로 접근하는것이 가능하다.
...
※ 기본적으로 프로그래머는 안정을 위해서 마음대로 메모리 상의 어떤 위치든 접근하는것이 불가능하다.

- 변수의 할당을 통해서 메모리의 사용 권한이 주어진 위치만 포인터를 통해서 조작하는 것이 가능

▶ C / C++ 언어 포인터에 자료형이 필요한 이유

- 포인터는 단순히 메모리의 주소에 불과하기 때문에 해당 정보만을 가지고는 메모리를 얼마만큼의 크기로 값을 저장하거나 읽어들일지 판단할 수 없다.

- 그렇기때문에 포인터의 자료형을 통해서 저장 및 읽어들일 메모리의 크기를 명시해 줘야 한다.

- 주의
※ 따라서 포인터와 해당 포인터에 저장 할 변수의 자료형은 일치해야 한다.

- 메모리의 주소만을 보관하고 싶다면 특정 자료형을 명시하는것이 아니라 void* 형으로 선언하는것도 가능하다.
...
- Call By Reference : 주소값의 전달 (참조에 의한 전달)
 ㄴ int add(int* a, int* b)
...
void main()
{
	...
	*/
	...
	- pValueC를 초기화 하지 않았다면 pValueC는 쓰레기 값을 가지고 있는데 이 값은 어디에서 쓰이고 있는 값일 수 도 있다.

	- visual stduio는 권한 최상위

	- 빈자리였다면 다행이지만 아니였다면 쓰이는 값이 변경되는것이므로 치명적인 오류가 발생한다.

	- 그래서 포인터가 가르키고 있는 값을 변경할때는 반드시 확인을 한 이후 변경하는 습관을 들여야 한다.
	 ㄴ C / C++ 프로그래머
	...
	*/
	...
	/*
	- NULL, nullptr의 의미는 현재 포인터가 아무런 메모리 위치도 가리크고 있지 않다는 뜻을 지니고 있다.
	  (포인터를 초기화하는데 주로 활용이 된다.)
	*/

	pValueC = &valueB;

	// 위험 코드  (없는 주소를 접근할 수 있는 가능성이 있다.)
	if (pValueC != nullptr)
	{
		// 널이 아닌 경우에만 변경한다.
		*pValueC = 30;
	...
	※ NULL은 진짜 널을 가르키는 포인터가 아니라 숫자 0을 의미한다.

	◈ nullptr
	ㄴ 널 포인터 값을 나타내는 포인터 리터럴
	ㄴ 포인터를 표현하는 값 중에 NULL을 표현하는 값이라고 할 수 있다.
	*/
	...
	// pArrA가 가르키는 첫번째 요소가 int형 싱글 포인터이니
	// 배열 이름 pArrA는 int형 더블 포인터가 된다.
	int* pArrA[3];
	...
	// numA의 주소값
	cout << "pNumA : " << pNumA << endl;
	
	// pNumA의 주소값
	cout << "pArrB : " << pArrB << endl;
	cout << "&pNumA : " << &pNumA << endl;

	// pArrB의 주소값
	cout << "dpArr : " << dpArr << endl;

	// pNumB의 주소값
	cout << "pArrB[1] : " << pArrB[1] << endl;
	...
	- 1. my String\0 -> 10개의 길이를 지닌 str1 배열
	 ㄴ 문자열 전체를 저장하는 배열

	- 2. 포인터 변수 str2에 저장이 된 your String의 주소값이 된다.
	 ㄴ 첫번째 문자를 단순히 가르키고만 있는 포인터 변수
	
	- 하지만 배열 이름 str1은 계속해서 'm'의 주소값이기 때문에 str1과 str2 문자열의 시작 주소값을 담고 있다는 의미에서는 동일하다.

	- 배열이름 str1은 계속해서 문자 'm'이 저장된 위치를 가르키는 상태여야 하지만 포인터 변수 str2는 다른 위치를 가르킬 수 있다.
	*/
	...
	- 전역 변수, 정적 변수 등 프로그램이 실행 될때 할당이 되고 종료될때 제거되는 영역
	...
	- 프로그램을 작성할때 메모리 필요량을 지정하는 정적 할당과는 달리 실행중에 필요한 만큼의 메모리를 할당하는 기법

	※ 컴파일 타입 또는 디자인 타입 != 런타임

	- 동적 할당된 메모리는 이름이 없는 변수라고 할 수 있다.

	- 점적인 메모리 영역을 차지하고 있으므로 일단 값을 기억할 수 있지만 이름이 없기 때문에 오직 포인터로만 접근할 수 있다.
	...
	- 만약 최대 100개의 데이터를 저장해야 할 때 우리는 정적으로 100개를 할당해야하지만 40개의 데이터밖에 들어오지 않았다면
	  60개는 낭비가 되는 상황이 생긴다.
	...
	*/
	...
	- 생성자 호출 여부
		ㄴ malloc은 C언어의 동적 할당 이기 떄문에 클래스를 고려하지 않았다.
			ㄴ 클래스를 고려하지 않았기 때문에 클래스 호출 시 클래스 생성자를 불러오지 않는다. (오류가 일어남)

	- malloc을 쓰는 경우도 있음
		ㄴ malloc은 클래스를 고려하지 않았기 때문에 클래스인지 여부를 확인하는 작업이 필요없다.
			ㄴ 따라서 함수호출 시 new보다 malloc이 빠르다. (경우에 따라 따로따로 사용하는 것이 효율적)
	...
	2. 댕글링 포인터 (Dangling Pointer)
	 ...
	 ㄴ 메모리 접근 시 예측 불가능한 결과를 유발하는 1등 공신


	※ 조숙한 해제 / 너무 성급한 해제  ->  댕글링 포인터와 같은 의미

	 ㄴ 보통 메모리 해제 후, 해제된 메모리에 접근하거나 함수 호출에서 자동 변수를 가르키는 포인터를 반환할때 자주 발생한다.

	 ㄴ 댕글링 포인터 자체가 잠재적인 보안 위험 등을 항상 안고 다니기 때문에 절대 생기면 안되는 포인터중에 하나
...
/*
과제 1. 자신이 만들 포트폴리오 구상

- 어떤 방향성을 가지고 게임을 만들것인가?

- 턴제 RPG / 슈팅 게임 / 방탈출 / 무한의 계단(탑 오르기)

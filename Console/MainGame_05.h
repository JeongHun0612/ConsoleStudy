...
// STL 3대장 벡터/리스트/맵 - 쓰임이 가장 많고 다른 STL의 상위호환인 경우가 많다.
...
- 검색속도가 빠르다. (읽기 능력이 탁월하다는 뜻)

- 반면에 입출력은 느리다. (벡터의 큰 단점)
...
- 컬렉션 클래스에 보관되어 있는 각 데이터에 접근하기 위한 포인터 객체로써
  STL에 포함되어 있는 모든 컬렉션 클래스는 데이터의 시작과 끝을 나타내는 begin과 end 함수를 지니고 있다.

- begin과 end 함수를 지니고 있기 때문에 루프를 반복해서 컬렉션에 포함되어 있는 모든 데이터에 접근하는 것이 가능하다.

EX)

for (auto Iterator = ValueListA.begin();
Iterator != ValueListA.end(); ++Iterator) // 포인터를 사용하기 때문에 전위/후위 여부가 굉장히 중요
{
	! Do Something
}
...
public:
	void printVector();
	inline void LinePrint() { cout << "===============================" << endl; } // inline 함수도 함수의 선언과 구현을 구별하지 않는다.
